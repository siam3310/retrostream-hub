  ...
  if (loading) {
    return (
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
        {[...Array(8)].map((_, i) => (
          <div key={i} className="rounded-lg border-2 border-foreground">
            <div className="aspect-[2/3] animate-pulse bg-muted rounded-t-lg" />
            <div className="border-t-2 border-foreground p-3">
              <div className="h-5 w-3/4 animate-pulse bg-muted rounded-md" />
              <div className="mt-2 h-4 w-1/2 animate-pulse bg-muted rounded-md" />
            </div>
          </div>
        ))}
      </div>
