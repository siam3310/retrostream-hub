...
const MovieDetail = () => {
  ...
  const [movie, setMovie] = useState<MovieOrSeries | null>(null);
  ...
  useEffect(() => {
    const fetchMovie = async () => {
      ...
      if (data) {
        setMovie(data as unknown as MovieOrSeries);
      }
      setLoading(false);
    };
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-4xl px-4 py-6">
        ...
        <VideoPlayer sources={movie.videoSources} poster={movie.backdropUrl} />
        ...
        <div className="mt-6 space-y-4">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            <Button
              variant={isInWatchLater(movie.id) ? 'default' : 'outline'}
              className="border-2 border-foreground"
              onClick={handleWatchLater}
            >
              <Clock className="mr-2 h-4 w-4" />
              {isInWatchLater(movie.id) ? 'In Watch Later' : 'Watch Later'}
            </Button>
          </div>
